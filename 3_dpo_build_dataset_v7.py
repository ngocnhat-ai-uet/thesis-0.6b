...
def is_stop_pair(pair: tuple[dict[str, Any], dict[str, Any]]) -> bool:
    return matcher.to_text(pair_finish_reason(pair)).strip().lower() == "stop"
...
def choose_positive_pair(
    pairs: list[tuple[dict[str, Any], dict[str, Any]]],
    rng: random.Random,
) -> tuple[dict[str, Any], dict[str, Any]]:
    stop_pairs = [pair for pair in pairs if is_stop_pair(pair)]
    return rng.choice(stop_pairs or pairs)


def choose_zero_rejected_pair(
    pairs: list[tuple[dict[str, Any], dict[str, Any]]],
    chosen_pair: tuple[dict[str, Any], dict[str, Any]],
) -> tuple[dict[str, Any], dict[str, Any]]:
    stop_pairs = [pair for pair in pairs if is_stop_pair(pair)]
    candidate_pairs = stop_pairs or pairs
    return closest_length_pair(candidate_pairs, pair_token_length(chosen_pair))
...
def build_dpo_dataset(
    ...
    for record in dataset_rows:
        ...
        if positive_pairs:
            chosen_pair = choose_positive_pair(positive_pairs, rng)
            chosen_vr_score = 1
        else:
        ...
        if zero_pairs:
            rejected_pair = choose_zero_rejected_pair(zero_pairs, chosen_pair)
            rejected_vr_score = 0
