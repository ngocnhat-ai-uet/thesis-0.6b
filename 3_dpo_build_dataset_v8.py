...
def closest_pair_by_token_difference(
    chosen_pairs: list[tuple[dict[str, Any], dict[str, Any]]],
    rejected_pairs: list[tuple[dict[str, Any], dict[str, Any]]],
) -> tuple[tuple[dict[str, Any], dict[str, Any]], tuple[dict[str, Any], dict[str, Any]]]:
    return min(
        (
            (chosen_pair, rejected_pair)
            for chosen_pair in chosen_pairs
            for rejected_pair in rejected_pairs
        ),
        key=lambda pair: (
            abs(pair_token_length(pair[0]) - pair_token_length(pair[1])),
            pair_token_length(pair[0]),
            pair_token_length(pair[1]),
        ),
    )


def normalized_finish_reason(pair: tuple[dict[str, Any], dict[str, Any]]) -> str:
    return matcher.to_text(pair_finish_reason(pair)).strip().lower()


def pairs_with_finish(
    pairs: list[tuple[dict[str, Any], dict[str, Any]]],
    finish: str,
) -> list[tuple[dict[str, Any], dict[str, Any]]]:
    return [pair for pair in pairs if normalized_finish_reason(pair) == finish]
...
def make_dpo_entry(
    record: dict[str, Any],
    prompt: str,
    chosen_pair: tuple[dict[str, Any], dict[str, Any]],
    chosen_vr_score: int,
    rejected_pair: tuple[dict[str, Any], dict[str, Any]],
    rejected_vr_score: int,
...
def select_zero_pair(
    positive_pairs: list[tuple[dict[str, Any], dict[str, Any]]],
    zero_pairs: list[tuple[dict[str, Any], dict[str, Any]]],
    rng: random.Random,
) -> tuple[
    tuple[dict[str, Any], dict[str, Any]],
    tuple[dict[str, Any], dict[str, Any]],
    ...
    positive_stop_pairs = pairs_with_finish(positive_pairs, "stop")
    positive_length_pairs = pairs_with_finish(positive_pairs, "length")
    zero_stop_pairs = pairs_with_finish(zero_pairs, "stop")
    zero_length_pairs = pairs_with_finish(zero_pairs, "length")

    if positive_stop_pairs and zero_stop_pairs:
        chosen_pair, rejected_pair = closest_pair_by_token_difference(
            positive_stop_pairs,
            zero_stop_pairs,
        )
        return (
    ...
    if positive_stop_pairs and zero_length_pairs:
        return (
            longest_pair(positive_stop_pairs),
            rng.choice(zero_length_pairs),
            "1,0:stop,length",
        )
    if positive_length_pairs and zero_length_pairs:
        return (
            rng.choice(positive_length_pairs),
            rng.choice(zero_length_pairs),
            "1,0:length,length",
        )
    if positive_length_pairs and zero_stop_pairs:
        return (
            rng.choice(positive_length_pairs),
            longest_pair(zero_stop_pairs),
            "1,0:length,stop",
        )
    return None


def select_negative_pair(
    positive_pairs: list[tuple[dict[str, Any], dict[str, Any]]],
    negative_pairs: list[tuple[dict[str, Any], dict[str, Any]]],
    rng: random.Random,
) -> tuple[
    tuple[dict[str, Any], dict[str, Any]],
    tuple[dict[str, Any], dict[str, Any]],
    ...
    positive_length_pairs = pairs_with_finish(positive_pairs, "length")
    negative_length_pairs = pairs_with_finish(negative_pairs, "length")

    if positive_length_pairs and negative_length_pairs:
        return (
            rng.choice(positive_length_pairs),
            rng.choice(negative_length_pairs),
            "1,-1:length,length",
        )
    return None
...
def build_dpo_dataset(
    ...
    dpo_entries: list[tuple[dict[str, Any], int, int]] = []
    ...
    for record in dataset_rows:
        ...
        selected_pair = select_zero_pair(positive_pairs, zero_pairs, rng)
        rejected_vr_score = 0
        if selected_pair is None:
            selected_pair = select_negative_pair(positive_pairs, negative_pairs, rng)
            rejected_vr_score = -1
        ...
        chosen_pair, rejected_pair, selection_group = selected_pair
        selection_group_counts[selection_group] += 1
        dpo_entries.append(
            make_dpo_entry(
                record,
                prompt,
    ...
    dpo_rows = [entry[0] for entry in dpo_entries]
    ...
    for row, chosen_token_length, rejected_token_length in dpo_entries:
        chosen_vr_score = int(row["chosen_vr_score"])
        rejected_vr_score = int(row["rejected_vr_score"])
    ...
    stats = {
        ...
        "selection_group_counts": {
            key: selection_group_counts[key] for key in sorted(selection_group_counts)
        },
    ...
    print(f"Selection groups: {dict(sorted(selection_group_counts.items()))}")
    if skip_reasons:
