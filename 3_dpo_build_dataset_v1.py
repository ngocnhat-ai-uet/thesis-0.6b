...
def extracted_last_boxed_answer(text: Any) -> str | None:
    answer = matcher.find_last_boxed_answer(text)
    if not answer.found:
        return None
...
def build_dpo_dataset(
    ...
    for record in dataset_rows:
        ...
        for pair in valid_pairs:
            grouped_pairs[
                generation_score(
                    record.get("label", pair[0].get("label")),
                    pair[1].get("model_output"),
                    prompt,
                )
            ].append(pair)
        ...
        if negative_pairs:
            ...
            zero_rejected_pool = zero_pairs
            if chosen_vr_score == 0:
                zero_rejected_pool = [pair for pair in zero_pairs if pair is not chosen_pair]
            if zero_rejected_pool:
                rejected_pair = rng.choice(zero_rejected_pool)
                rejected_vr_score = 0
        ...
        rejected_answer = optional_text(extracted_last_boxed_answer(rejected_generation.get("model_output")))
        dpo_rows.append(
            {
                "index": record["index"],
    ...
    stats = {
        ...
        "chosen_score_counts": {str(key): chosen_score_counts[key] for key in sorted(chosen_score_counts)},
